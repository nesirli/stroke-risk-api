...
def interpret_model(X: pd.DataFrame, y: pd.Series) -> None:
    ...
    feature_names = [
        re.sub(r"^(num__|cat__)", "", f) for f in preprocessor.get_feature_names_out()
    ]
    ...
    plt.savefig(
        settings.plot_dir / "log_reg_beeswarm.png", dpi=150, bbox_inches="tight"
    )
    plt.close()
    ...
    for col in ["age", "avg_glucose_level"]:
        ...
        plt.savefig(
            plot_dir / f"log_reg_scatter_{col}.png", dpi=150, bbox_inches="tight"
        )
        plt.close()
